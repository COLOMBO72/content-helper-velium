...
const FROM_DATE = new Date(process.env.MIGRATOR_FROM_DATE || '2025-01-01T00:00:00Z');
...
bot.onText(/\/status/, (msg) => {
  ...
  const lastAt = progress.lastPostedAt
    ? new Date(progress.lastPostedAt).toLocaleString('ru-RU', { timeZone: 'Europe/Moscow' })
    : 'ещё не было';
...
bot.onText(/\/pause/, (msg) => {
  if (msg.from.id !== YOUR_TG_ID) return;
  paused = true;
  bot.sendMessage(msg.chat.id, '⏸ Миграция поставлена на паузу.\n\nНапиши /resume чтобы продолжить.');
});

bot.onText(/\/resume/, (msg) => {
  if (msg.from.id !== YOUR_TG_ID) return;
  paused = false;
  bot.sendMessage(msg.chat.id, '▶️ Продолжаем! Следующий пост придёт по расписанию.');
  scheduleNext(true);
});
...
bot.onText(/\/next/, (msg) => {
  ...
  if (waitingForApproval) {
    bot.sendMessage(msg.chat.id, '⏳ Сначала одобри или пропусти текущий пост.');
    return;
  }
...
bot.onText(/\/skip (\d+)/, (msg, match) => {
  ...
  progress.currentIndex = Math.min(progress.currentIndex + n, progress.posts.length);
  saveProgress(progress);
  bot.sendMessage(msg.chat.id, `⏭ Пропущено ${n} постов. Текущий индекс: ${progress.currentIndex}`);
});
...
bot.on('message', (msg) => {
  ...
  const channelUsername = fwdChat.username ? `@${fwdChat.username}` : String(fwdChat.id);
  ...
  if (msg.poll) {
    bot.sendMessage(msg.chat.id, `⏭ Пост с опросом пропущен (в ВК не поддерживается)`);
    return;
  }
  ...
  // Проверяем дубликаты
  const isDuplicate = progress.posts.some(p => p.messageId === postData.messageId);
...
async function sendNextPost(chatId) {
  ...
  const header = `<b>📋 Пост ${progress.currentIndex + 1} из ${progress.posts.length}</b>\n` +
    `📅 ${postDate}\n` +
    `━━━━━━━━━━━━━━━━\n`;
  ...
  try {
    if (post.hasPhoto && post.photoFileId) {
      ...
      const caption = header + (post.text ? post.text.slice(0, 800) : '(без текста)') + footer;
      ...
      // Только текст
      const text = header + (post.text ? post.text.slice(0, 3000) : '(пустой пост)') + footer;
    ...
    bot.sendMessage(chatId, `⚠️ Не удалось загрузить пост ${progress.currentIndex + 1}, пропускаю...`);
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'mg_reset_confirm') {
    ...
    saveProgress(progress);
    waitingForApproval = false;
    bot.sendMessage(chatId, '🔄 Прогресс сброшен. Пересылай посты боту заново.');
    return;
  }
...
async function postToVK(text, photoPath) {
  ...
  if (photoPath && fs.existsSync(photoPath)) {
    ...
    const serverRes = await axios.get(`${VK_API}/photos.getWallUploadServer`, {
      params: { group_id: VK_GROUP_ID, access_token: VK_TOKEN, v: VK_V },
    });
...
function cleanupPhoto() {
  if (pendingPhotoPath && fs.existsSync(pendingPhotoPath)) {
    try { fs.unlinkSync(pendingPhotoPath); } catch {}
  }
...
setTimeout(() => {
  bot.sendMessage(YOUR_TG_ID,
    ...
    { parse_mode: 'HTML' }
  ).catch(e => console.error('Не могу написать тебе в личку. Убедись что ты написал боту /start'));

  // Если есть посты и не на паузе — запускаем расписание
  if (progress.posts.length > 0 && progress.currentIndex < progress.posts.length) {
    scheduleNext(false);
  }
}, 2000);
