...
// ── Инициализируем БД, затем запускаем всё остальное ─────────────
db.initDb()
  .then(() => {
...
bot.on("channel_post", async (msg) => {
  ...
  console.log(
    `[TG] Новый пост из ${channelUsername}, пользователей: ${users.length}`,
  );
  ...
  for (const user of users) {
    try {
      ...
      console.log(
        `[Queue] user=${user.telegram_id} добавлено для ${count} платформ`,
      );
    } catch (e) {
...
function findUsersWithChannel(channelUsername) {
  ...
  const rows = db.getAllActiveSettings();
